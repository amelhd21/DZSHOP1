...
// En local : http://localhost:5000. En ligne : la variable VITE_API_URL (voir séance 17).
...
export async function apiFetch(chemin, options) {
  ...
  // Sauf pour les fichiers (FormData), on envoie du JSON
  ...
  const pageAuth = chemin.startsWith('/api/auth/login') || chemin.startsWith('/api/auth/register')
